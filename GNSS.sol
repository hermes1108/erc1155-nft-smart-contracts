...
abstract contract gnssInterface {
    function mintTransfer(address to, string calldata hash)
        public
        virtual
        returns (uint256);
}
...
contract GNSS is ERC1155, Ownable, ERC1155Burnable {
    ...
    constructor()
        ERC1155("ipfs://QmczVPwCRwYuLB7gMf937SWGK6HBF61Wns8rud4AGstZja")
    {}
    ...
    function mint(uint256 amountToMint) public payable returns (uint256) {
        ...
        require(
            amountToMint <= 10,
            "You can't mint more than 10 per transaction!"
        );
    ...
    function migrateToken(uint256 id, string calldata hash)
        public
        returns (uint256)
    {
